#' @title Harmonize Publisher in Kungliga
...
#' @keywords utilities
harmonize_publisher_kungliga <- function(x, publication_year, languages=c("english")) {
  ...
  tab <- tab[!is.na(tab[,1]),]
  nams <- as.character(tab[,1])
  compPublisher <- as.character(compPublisher)
  personal_names$init_name <- as.character(personal_names$init_name)

  for (i in 1:length(nams)) {  

    message(paste("harmonize_publisher_kungliga", i/length(nams)))

    publisherName <- nams[[i]]

    # Publisher names are unique in tab[,1] so no need to search for the index:
    # tab[,1]==publisherName -> i

    publisherTotal <- tab[i, 2]
    compare_version <- compPublisher[i]
    ...
    inds4 <- ranges$publisher==publisherName
    pub_start <- ranges$min[inds4]
    pub_end <- ranges$max[inds4]
    ...
    inds5 <- grepl(familyname, personal_names$family)
    inds6 <- personal_names$guessed
    ...
    if ((!is.na(initials)) && (!is.na(guessed)) && (!guessed)) {
      ...
      inds3 <- which(framePublishers$comp %in% ignore_publishers)
      if (length(ignore_publishers) > 0) {
        ignore_indices <- inds3
        tmp_compare_versions[ignore_indices] <- NA
      }
      ...
      indices <- which(!inds5 & inds6)
      ignore_indices <- indices # intersect(indices, which(personal_names$guessed))
      tmp_compare_versions[ignore_indices] <- NA
      
      # Insert the comparable version with initials instead of the full names
      fix_indices <- which(inds5 & inds6)
      fix_indices <- intersect(fix_indices, which(!is.na(tmp_compare_versions)))
      tmp_compare_versions[fix_indices] <- personal_names$init_name[fix_indices]
      ...
      ignore_indices <- which(personal_names$initials != initials)
      tmp_compare_versions[ignore_indices] <- NA
      ignore_indices <- intersect(grep(familyname, personal_names$family, invert=TRUE), which(!personal_names$guessed))
      tmp_compare_versions[ignore_indices] <- NA
      ...
      fix_indices <- which(inds5 & !personal_names$guessed & !is.na(tmp_compare_versions))
    ...
    if ((is.null(res)) || (is.na(res)) || (res=="")) {
      ...
      inds2 <- which(framePublishers$orig==res)
      res <- framePublishers$mod[inds2]
      # Add combined total for the publisher name already found
      combinedTotal <- unlist(framePublishers$total[inds2], use.names=FALSE) + publisherTotal
      framePublishers$total[inds2] <- combinedTotal
      ...
      inds <- which(ranges$publisher %in% involved_publisher_names)
      ranges$min[inds] <- min(ranges$min[inds], pub_start) # new start year 
      ranges$max[inds] <- max(ranges$max[inds], pub_end) # new_end_year
